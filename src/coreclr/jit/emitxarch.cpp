...
emitter::code_t emitter::AddRexBPrefix(instruction ins, code_t code)
{
    ...
    if (UseVEXEncoding() && IsAVXInstruction(ins))
    {
        if (TakesVexPrefix(ins))
        {
            // B-bit is available in 3-byte VEX prefix that starts with byte C4.
            assert(hasVexPrefix(code));
...
// Outputs EVEX prefix (in case of AVX512 instructions), VEX prefix (in case of AVX instructions) and REX.R/X/W/B
// otherwise.
...
//------------------------------------------------------------------------
// emitGetAdjustedSizeSIMD: Determines any size adjustment needed for a given instruction based on the current
// configuration.
//
// Arguments:
...
unsigned emitter::emitGetAdjustedSizeSIMD(instruction ins, emitAttr attr, code_t code)
{
...
// Returns true iff the given CPU instruction has an tuple type info
...
/*****************************************************************************
 *
 *  Returns the tuple type info for a given CPU instruction
 */
...
inline UNATIVE_OFFSET emitter::emitInsSizeRR(instrDesc* id, code_t code)
{
    ...
    UNATIVE_OFFSET sz = emitGetAdjustedSizeSIMD(ins, attr, code);
    ...
    else
    {
        ...
        else
        {
            ...
            if (EBPbased)
            {
                ...
                // TODO-XARCH-ANTHONY: Have to take into account whether EVEX prefex will
                // adjust offset
                if (TakesEvexPrefix(id->idIns()))
                {
                    bool compressedFitsInByte = false;
                    AdjustDispForEvex(id, ssize_t(offs), &compressedFitsInByte);
...
inline UNATIVE_OFFSET emitter::emitInsSizeSV(instrDesc* id, code_t code, int var, int dsp)
{
    ...
    UNATIVE_OFFSET prefix   = emitGetAdjustedSizeSIMD(ins, attrSize, code);
...
UNATIVE_OFFSET emitter::emitInsSizeAM(instrDesc* id, code_t code)
{
    ...
    else
    {
        if (TakesEvexPrefix(ins))
        {
            dsp = AdjustDispForEvex(id, dsp, &dspInByte);
        }
    }
    ...
    size += emitGetAdjustedSizeSIMD(ins, attrSize, code);
...
BYTE* emitter::emitOutputAM(BYTE* dst, instrDesc* id, code_t code, CnsVal* addc)
{
    ...
    if ((ins == INS_call) || (ins == INS_tail_i_jmp))
    {
        ...
        if (id->idIsCallRegPtr())
        {
            ...
            // TODO-XARCH-ANTHONY this change is merely a consistency one, which brings a question: do we want to
            // swap all uses of emitOutputRexOrVexPrefixIfNeeded to emitOutputSIMDPrefixIfNeeded etc, to start
    ...
    // TODO-XARCH-ANTHONY come back and evaluate the addc branch
    ...
    {
        // TODO-XARCH-ANTHONY come back and evaluate the special case branch
...
BYTE* emitter::emitOutputSV(BYTE* dst, instrDesc* id, code_t code, CnsVal* addc)
{
    ...
    if (TakesEvexPrefix(ins))
    {
        ssize_t dsp2 = ssize_t(dsp);
        dsp = int(AdjustDispForEvex(id, ssize_t(dsp), &dspInByte));
    }
    else
    {
        dspInByte = ((signed char)dsp == (ssize_t)dsp);
...
ssize_t emitter::AdjustDispForEvex(instrDesc* id, ssize_t dsp, bool *dspInByte)
{
    ...
    *dspInByte = ((signed char)compressedDsp == (ssize_t)dsp);
    if (*dspInByte)
    {
...
size_t emitter::emitOutputInstr(insGroup* ig, instrDesc* id, BYTE** dp)
{
    ...
    switch (id->idInsFmt())
    {
        ...
        case IF_RWR_RRD_SRD:
        {
            ...
            // TODO-XARCH-ANTHONY: Need to put the proper instruction here (IsSIMDInstruction(ins) probably
            // after Deepak updates
            assert(IsAVXInstruction(ins));
