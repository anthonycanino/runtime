...
// - Uses Operand mask encoding: 64-bit opmask registers k0-k7 for conditional execution and merging of destination operands.
// - Need to encode functionality specific to Instruction classes(e.g.,embedded broadcast, embedded rounding control etc.)
...
unsigned emitter::emitGetAdjustedSizeSIMD(instruction ins, emitAttr attr, code_t code)
{
    ...
    // IsAVX512Instruction(ins) is TRUE for AVX/SSE instructions also which needs to be VEX encoded unless explicitly asked for EVEX.
...
inline UNATIVE_OFFSET emitter::emitInsSizeRR(instrDesc* id, code_t code, int val)
{
    ...
    {
        assert(!IsSSEOrAVXorAVX512Instruction(ins));
    }
...
inline UNATIVE_OFFSET emitter::emitInsSizeRR(instruction ins, regNumber reg1, regNumber reg2, emitAttr attr)
{
    ...
    UNATIVE_OFFSET sz = emitGetAdjustedSizeSIMD(ins, size, insCodeRM(ins));
    ...
    if ((code & 0xFF00) != 0)
    {
        sz += IsSSEOrAVXorAVX512Instruction(ins) ? emitInsSize(code, includeRexPrefixSize) : 5;
    }
    else
    {
...
BYTE* emitter::emitOutputRR(BYTE* dst, instrDesc* id)
{
    ...
    else if ((ins == INS_movsx) || (ins == INS_movzx) || (insIsCMOV(ins)))
    {
        ...
        code = insCodeRM(ins);
        // Should never come into this path for EVEX or VEX(??)
        code = AddSIMDPrefixIfNeeded(ins, code, size);
    ...
    {
        assert(!TakesVexPrefix(ins) && !TakesEvexPrefix(ins));
        code = insCodeMR(ins);
    ...
    if (TakesVexPrefix(ins) || TakesEvexPrefix(ins))
    {
